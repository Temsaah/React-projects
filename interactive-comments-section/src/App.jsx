  ...
  return (
    <div className="col-span-2 row-start-2 self-center sm:col-start-2">
      {isEditing && currentCommentID === comment.id ? (
        <UpdateComment content={comment.content} commentID={comment.id} />
      ) : (
...
function ConfirmDeleteModal() {
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-10 grid h-full w-full place-items-center bg-black/80"
        onClick={(e) =>
          e.target === e.currentTarget && setShowDeleteModal(false)
        }
      >
        <div className="delete-comment-container z-20 mx-4 grid max-w-[400px] gap-5 rounded-lg bg-white p-7">
